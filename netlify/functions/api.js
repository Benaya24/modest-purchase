...
// **FIXED: Explicitly provide siteID and token for authentication**
// This ensures the function can always connect to the correct Blob store.
const getProductsStore = () => getStore({
...
exports.handler = async function(event) {
    ...
    if (!APP_KEY || !APP_SECRET || !TRACKING_ID) {
        return { statusCode: 500, body: JSON.stringify({ error: "Server configuration error: Missing API keys or Tracking ID in environment variables." }) };
    }
    ...
    try {
        ...
        if (event.httpMethod === 'POST' && action === 'removeProduct') {
            ...
            if (products[category]) {
                products[category] = products[category].filter(id => id !== productId);
            }
            await getProductsStore().setJSON('all', products);
        ...
        if (event.httpMethod === 'POST' && action === 'getProductDetails') {
            ...
            if (!productIds || productIds.length === 0) return { statusCode: 400, body: JSON.stringify({ error: 'No product IDs' }) };
            ...
            const params = {
                ...
                partner_id: 'cainiao',
                fields: 'product_id,product_title,target_sale_price,product_main_image_url,discount,evaluate_rate,original_price,promotion_link',
